...
ERROR_RESPONSES = {
    404: {
        ...
        "description": "Tender, supplier, quote, item or comparison not found",
    },
    409: {
        "model": ErrorResponseSchema,
        "description": "Invalid state or duplicate quote",
    },
    413: {
        "model": ErrorResponseSchema,
        "description": "Quote document exceeds configured limit",
    },
    422: {
        "model": ErrorResponseSchema,
        "description": "Invalid quote document or review payload",
    },
    503: {
        "model": ErrorResponseSchema,
        "description": "Storage, AI or queue unavailable",
    },
}
...
async def upload_quote(
    ...
    uploaded_by_user_id, supplier_id, rfq_request_id, uploads = (
        await parse_quote_upload(
            request,
...
def list_tender_quotes(
    ...
    return TenderQuotesResponseSchema(
        items=tuple(
            QuoteResponseSchema.model_validate(item, from_attributes=True)
            for item in items
        ),
        total=len(items),
    )
...
def get_quote(
    ...
    result = GetQuote(uow_factory).execute(quote_id)
    return QuoteResponseSchema.model_validate(result, from_attributes=True)
...
def process_quote(
    ...
    result = GetQuoteProcessingStatus(uow_factory).execute(quote_id)
...
@router.get(
    "/quotes/{quote_id}/processing-status",
    response_model=QuoteProcessingStatusResponseSchema,
    responses={404: ERROR_RESPONSES[404]},
)
def quote_processing_status(
    quote_id: UUID,
    uow_factory: UowFactoryDependency,
) -> QuoteProcessingStatusResponseSchema:
    result = GetQuoteProcessingStatus(uow_factory).execute(quote_id)
...
@router.get(
    ...
    response_model=QuoteItemsResponseSchema,
    responses={404: ERROR_RESPONSES[404]},
)
def quote_items(
    quote_id: UUID,
...
@router.get(
    "/quotes/{quote_id}/evidence",
    response_model=QuoteEvidenceListResponseSchema,
    responses={404: ERROR_RESPONSES[404]},
)
def quote_evidence(
    quote_id: UUID,
...
def approve_quote(
    ...
    result = ApproveQuote(uow_factory).execute(
        quote_id,
        request.reviewer_user_id,
    )
    return QuoteResponseSchema.model_validate(result, from_attributes=True)
...
def reprocess_quote(
    ...
    result = ReprocessQuote(uow_factory, queue).execute(
        quote_id,
