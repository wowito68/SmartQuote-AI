...
from app.api.quote_reception_schemas import (
    ProcessingStatusResponseSchema, QuoteDetailResponseSchema, QuoteDocumentsResponseSchema,
    QuoteItemsResponseSchema, QuoteItemUpdateSchema, QuoteProcessingRequestSchema,
    QuoteReviewActionSchema, QuoteUploadResponseSchema,
)
from app.application.dtos.quote_reception import AddQuoteDocumentCommand, ReceiveQuoteCommand, UpdateQuoteItemCommand
...
from app.application.use_cases.quote_reception import (
    AddQuoteDocument, ApproveQuote, GetProcessingStatus, GetQuoteDetail, QueueQuoteProcessing,
    ReceiveQuote, RejectQuote, SubmitQuoteForReview, UpdateQuoteItem,
)
...
async def receive_quote(
    ...
    user_id, files = await parse_document_upload(
        request, maximum_file_size_bytes=settings.max_document_size_bytes, maximum_files=1
    )
    if len(files) != 1:
        raise InvalidDocumentFile("Exactly one quote document is required.")
    result, duplicate = ReceiveQuote(
        uow_factory, storage, queue, maximum_size_bytes=settings.max_document_size_bytes
    ).execute(ReceiveQuoteCommand(
        tender_id=tender_id, tender_supplier_id=tender_supplier_id,
        uploaded_by_user_id=user_id, file=files[0], rfq_request_id=rfq_request_id,
        correlation_id=x_correlation_id,
    ))
    return QuoteUploadResponseSchema(
        quote=QuoteDetailResponseSchema.model_validate(result, from_attributes=True), duplicate=duplicate
    )


@router.post("/quotes/{quote_id}/documents", response_model=QuoteDocumentsResponseSchema, status_code=201)
async def add_quote_document(
    quote_id: UUID, request: Request, uow_factory: UowFactoryDependency,
    storage: FileStorageDependency, settings: SettingsDependency,
    ...
    if len(files) != 1:
        raise InvalidDocumentFile("Exactly one quote document is required.")
    AddQuoteDocument(uow_factory, storage, maximum_size_bytes=settings.max_document_size_bytes).execute(
        AddQuoteDocumentCommand(quote_id=quote_id, uploaded_by_user_id=user_id, file=files[0])
    )
...
@router.post("/quotes/{quote_id}/process", response_model=ProcessingStatusResponseSchema, status_code=202)
def process_quote(
    quote_id: UUID, payload: QuoteProcessingRequestSchema, uow_factory: UowFactoryDependency,
    queue: QuoteQueueDependency, x_correlation_id: Annotated[str | None, Header(alias="X-Correlation-ID")] = None,
    ...
    QueueQuoteProcessing(uow_factory, queue).execute(quote_id, x_correlation_id, force=False)
    return ProcessingStatusResponseSchema.model_validate(GetProcessingStatus(uow_factory).execute(quote_id), from_attributes=True)


@router.get("/quotes/{quote_id}/processing-status", response_model=ProcessingStatusResponseSchema)
def processing_status(quote_id: UUID, uow_factory: UowFactoryDependency) -> ProcessingStatusResponseSchema:
    return ProcessingStatusResponseSchema.model_validate(GetProcessingStatus(uow_factory).execute(quote_id), from_attributes=True)
...
def update_quote_item(
    quote_id: UUID, item_id: UUID, payload: QuoteItemUpdateSchema,
    uow_factory: UowFactoryDependency,
...
def submit_quote_review(
    ...
    result = SubmitQuoteForReview(uow_factory).execute(quote_id, payload.requested_by_user_id)
    return QuoteDetailResponseSchema.model_validate(result, from_attributes=True)


@router.post("/quotes/{quote_id}/approve", response_model=QuoteDetailResponseSchema)
def approve_quote(
    quote_id: UUID, payload: QuoteReviewActionSchema, uow_factory: UowFactoryDependency,
) -> QuoteDetailResponseSchema:
...
def reject_quote(
    ...
    result = RejectQuote(uow_factory).execute(quote_id, payload.reviewer_user_id, payload.reason)
    return QuoteDetailResponseSchema.model_validate(result, from_attributes=True)


@router.post("/quotes/{quote_id}/reprocess", response_model=ProcessingStatusResponseSchema, status_code=202)
def reprocess_quote(
    quote_id: UUID, payload: QuoteProcessingRequestSchema, uow_factory: UowFactoryDependency,
    queue: QuoteQueueDependency, x_correlation_id: Annotated[str | None, Header(alias="X-Correlation-ID")] = None,
) -> ProcessingStatusResponseSchema:
    ...
    QueueQuoteProcessing(uow_factory, queue).execute(quote_id, x_correlation_id, force=True)
    return ProcessingStatusResponseSchema.model_validate(GetProcessingStatus(uow_factory).execute(quote_id), from_attributes=True)
