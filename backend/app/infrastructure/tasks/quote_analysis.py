...
from app.application.use_cases.quote_reception import ProcessQuote
from app.config.settings import get_settings
from app.domain.quotes.exceptions import QuoteExtractionFailure
from app.infrastructure.ai.openai_extraction_service import OpenAIExtractionService, OpenAIResponsesHTTPClient
...
from app.infrastructure.extraction.quote_document_extractors import (
    CompositeQuoteDocumentExtractor, DocxQuoteDocumentExtractor, PdfQuoteDocumentExtractor,
    XlsxQuoteDocumentExtractor,
)
...
def analyze_supplier_quote(self: Task, quote_id: str, force: bool = False) -> str:
    ...
    logger.info("quote_analysis_started", extra={"quote_id": quote_id, "correlation_id": correlation_id, "force": force})
    pdf = FallbackDocumentTextExtractor(
        PyMuPDFExtractor(), PdfPlumberExtractor(),
        ExtractionPolicy(
    ...
    extractor = CompositeQuoteDocumentExtractor(
        PdfQuoteDocumentExtractor(pdf), XlsxQuoteDocumentExtractor(), DocxQuoteDocumentExtractor()
    )
    ...
    run_id = ProcessQuote(
        SqlAlchemyUnitOfWork, LocalFileStorage(settings.storage_root), extractor, service,
        FilePromptRegistry(), prompt_version=settings.quote_ai_prompt_version,
        model=settings.ai_model, temperature=settings.ai_temperature,
    ).execute(UUID(quote_id), correlation_id, force=force)
    logger.info("quote_analysis_completed", extra={"quote_id": quote_id, "correlation_id": correlation_id, "extraction_run_id": str(run_id)})
    return str(run_id)
