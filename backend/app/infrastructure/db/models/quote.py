...
class QuoteExtractionRunModel(Base):
    ...
    id: Mapped[UUID] = mapped_column(Uuid, primary_key=True)
    quote_id: Mapped[UUID] = mapped_column(Uuid, ForeignKey("quotes.id", ondelete="CASCADE"), nullable=False)
    quote_document_id: Mapped[UUID | None] = mapped_column(Uuid, ForeignKey("quote_documents.id", ondelete="CASCADE"), nullable=True)
    tender_id: Mapped[UUID] = mapped_column(Uuid, ForeignKey("tenders.id", ondelete="CASCADE"), nullable=False)
    supplier_id: Mapped[UUID] = mapped_column(Uuid, ForeignKey("suppliers.id", ondelete="RESTRICT"), nullable=False)
    idempotency_key: Mapped[str] = mapped_column(String(64), nullable=False)
    extraction_fingerprint: Mapped[str | None] = mapped_column(String(64), nullable=True)
    ...
    extractor_name: Mapped[str] = mapped_column(String(100), nullable=False, default="unknown")
    ...
    reused_from_run_id: Mapped[UUID | None] = mapped_column(Uuid, ForeignKey("quote_extraction_runs.id", ondelete="SET NULL"), nullable=True)
    is_approved_source: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
...
class QuoteItemModel(Base):
    ...
    source_evidence_id: Mapped[UUID | None] = mapped_column(Uuid, ForeignKey("quote_evidence_references.id", ondelete="SET NULL"), nullable=True)
...
class QuoteTaskRecordModel(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("correlation_id", name="uq_quote_task_records_correlation"),
