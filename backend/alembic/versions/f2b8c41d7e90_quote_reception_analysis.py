...
def _replace_quote_status_constraint(values: str) -> None:
    bind = op.get_bind()
    if bind.dialect.name == "sqlite":
        with op.batch_alter_table("quotes", recreate="always") as batch:
            batch.drop_constraint("valid_quote_status", type_="check")
            batch.create_check_constraint("valid_quote_status", f"status IN ({values})")
    else:
        op.drop_constraint("valid_quote_status", "quotes", type_="check")
...
def upgrade() -> None:
    ...
    _replace_quote_status_constraint(
        "'received','validating','extracting','extracted','normalized','pending_review',"
        "'approved','rejected','failed','included_in_comparison'"
    )

    with op.batch_alter_table("quote_extraction_runs") as batch:
        batch.add_column(sa.Column("entity_type", sa.String(50), nullable=True))
        batch.add_column(sa.Column("provider", sa.String(100), nullable=True))
        batch.add_column(sa.Column("duration_ms", sa.Integer(), nullable=True))
    op.execute(
        sa.text(
            "UPDATE quote_extraction_runs SET entity_type='quote', provider='openai' "
            "WHERE entity_type IS NULL OR provider IS NULL"
        )
    )
    with op.batch_alter_table("quote_extraction_runs") as batch:
    ...
    op.create_table(
        ...
        sa.Column("extractor_version", sa.String(255), nullable=True),
    ...
    op.execute(
        sa.text(
            ...
            "(id,quote_id,storage_key,original_file_name,mime_type,document_type,processing_status,file_hash,file_size,created_at) "
            "SELECT id,id,storage_key,original_file_name,mime_type,'supplier_quote',"
            "CASE WHEN status IN ('extracted','normalized','pending_review','approved','included_in_comparison') "
            "THEN 'extracted' ELSE 'stored' END,file_hash,file_size,created_at FROM quotes"
        )
    )
    ...
    with op.batch_alter_table("quote_items") as batch:
        ...
        batch.add_column(sa.Column("compliance_status", sa.String(30), nullable=True))
        batch.add_column(sa.Column("match_status", sa.String(30), nullable=True))
    ...
    op.execute(
        sa.text(
            ...
            "compliance_status=CASE WHEN technical_compliance IS TRUE THEN 'compliant' "
            "WHEN technical_compliance IS FALSE THEN 'non_compliant' ELSE 'unknown' END,"
...
def downgrade() -> None:
    ...
    _replace_quote_status_constraint(
        "'received','validating','extracting','extracted','normalized','pending_review',"
        "'approved','rejected','included_in_comparison'"
    )
    with op.batch_alter_table("quotes") as batch:
